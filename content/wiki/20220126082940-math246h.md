...
author = ["ksam"]
date = 2022-01-26
lastmod = 2022-02-16T00:15:59-05:00
draft = false
slug = "MATH246H"
...
\\[\rho(x,y)M\_y(x,y) = \rho\_x(x,y)N(x,y) + \rho(x,y)N\_y(x,y)\\]
\\[\rho(x,y)M\_y(x,y) - \rho(x,y)N\_x(x,y) =  \rho\_x(x,y)N(x,y)\\]
\\[\rho(x,y) \frac{M\_y(x,y) - N\_y(x,y)}{N(x,y)} = \rho\_x(x,y)\\]

Similarly, if we set \\(\rho\_x\\) to 0:

\\[\rho\_y(x,y)M(x,y) + \rho(x,y)M\_y(x,y) = \rho(x,y)N\_y(x,y)\\]
\\[\rho\_y(x,y)M(x,y) = \rho(x,y)N\_y(x,y) - \rho(x,y)M\_y(x,y)\\]
\\[\rho\_y(x,y) = \rho(x,y) \frac{N\_y(x,y) - M\_y(x,y)}{M(x,y)}\\]
